...
var updateHourlyAvgValue = function(error, data) {
    ...
    // get all values for from start of current hour up to now
    // if it's 14:30, then get values between 14:00 and 14:30
    BitcoinValue.find({
        timestamp: {
            $gte: roundHourUTCtimestamp,
            $lte: UTCtimestamp
        }
    }, function(error, data) {
        // error check
        ...
        // check if there is an hourly avg value with this timestamp
        // update/create accordingly
        BitcoinValueHourlyAvg.update({
            timestamp: roundHourUTCtimestamp
        },
        { value: hourAvg},
        function (err, raw) {
            if(err) {
